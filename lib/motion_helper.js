...
this.set_control = function(data){
    ...
    switch (src_arr[2]) {
        ...
        case 'detection_start':
            var my_call = that.uri + '/' + my_dev + '/' + 'detection/start';
            request(my_call);
            break;
        case 'detection_pause':
            var my_call = that.uri + '/' + my_dev + '/' + 'detection/pause';
            request(my_call);
            break;
    }
    ...
    this.set_val = function(device,setting,myval,callback){

        request(that.uri + "/" + device + "/config/set?" + setting + "=" + myval, function (error, response, body) {
            if (error) {
                console.log(error);
            }else{
                request(that.uri + "/" + device + "/config/write", function (error,response,body){console.log("written : " + body); callback("written");});
            }
        })
    }
    ...
    this.get_val = function(device, setting, callback){
	...
	/*  var mydevice;
        ...
        console.log(device);
        //var res = get(that.uri + "/" + device + "/config/get?query=" + setting);
        request(that.uri + "/" + device + "/config/get?query=" + setting, function (error, response, body) {
            if (error) {
                console.log(error);
            }else{
                var conf_arr = body.split("\n");
                var mytuple = {};
                var key_val_arr = conf_arr[0].split("=");
                var mykey = key_val_arr[0].trim();
                    if (typeof key_val_arr[1] == 'string') {
                        var myval = key_val_arr[1].trim();
                    }else{
                        var myval = key_val_arr[1];
                    };
